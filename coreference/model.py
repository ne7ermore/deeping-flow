...
class Transformer:
    ...
    self.initializer = tf.random_normal_initializer(stddev=const.INIT_RANGE)

    with tf.compat.v1.variable_scope('init_variables'):

    self.src_max_len = tf.compat.v1.placeholder(tf.int32, name="src_max_len")
    self.batch_size = tf.compat.v1.placeholder(tf.int32, name="batch_size")
    self.tgt_max_len = tf.compat.v1.placeholder(tf.int32, name="tgt_max_len")
    ...
    with tf.compat.v1.variable_scope("encode", reuse=tf.compat.v1.AUTO_REUSE):
        # embedding
    enc_output = tf.nn.embedding_lookup(self.word_embedding, src_tensor)
    enc_output *= args.d_model**0.5

    enc_output += tf.nn.embedding_lookup(self.pos_embedding, src_postion)

    enc_output += tf.nn.embedding_lookup(self.turn_embedding, turns_tensor)
    ...
    distill_loss = tf.losses.mean_squared_error(dist_encode, enc_output)
    enc_output = tf.layers.dense(
    ...
    with tf.compat.v1.variable_scope("decode", reuse=tf.compat.v1.AUTO_REUSE):
        # decode embedding
    dec_output = tf.nn.embedding_lookup(self.word_embedding, tgt_tensor)
    ...
    last_enc_output = tf.expand_dims(last_enc_output, 0)  # 1 bsz slen dim

    dec_output_trans = tf.transpose(dec_output, [1, 0, 2])  # tlen bsz dim
    ...
    dec_output_trans = tf.expand_dims(dec_output_trans, 2)  # tlen bsz 1 dim
    ...
    rouge_score = common.rouge_l(predict, tgt_indexs_tensor, eos_indexs)
    ...
    def deeee(self, validation_data, sess, bert):
    (src_tensor, src_postion, turns_tensor), (tgt_tensor,
                                              tgt_postion), tgt_indexs_tensor, src_max_len, eos_indexs, tgt_max_len, src_context = next(validation_data)
